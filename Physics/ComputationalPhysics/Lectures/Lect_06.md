...
### 真随机数（True Random Number / Physical Random Number）

> A **true random number** is a number generated by measuring a physical entropy source whose unpredictability is guaranteed by the laws of physics rather than by algorithmic complexity. Its successive samples are **statistically independent and uncorrelated** at all lags (zero autocorrelation for any non-zero lag), and follow a known target distribution (typically uniform) up to the bias of the physical source.

> **NIST SP 800-90B:** "A non-deterministic random bit generator (NRBG) always has access to an entropy source and produces output bitstrings that have **full entropy** — often called a true random number generator."

利用物理熵源（热噪声、电路抖晃、放射性衰变、光子到达时间）生成。统计特征：序列元素**相互独立**（任意滞后自相关为零），分布由物理源经后处理逼近目标分布（通常为均匀分布 $U(0,1)$）。不可复现。由硬件随机数生成器（HRNG / TRNG）产生（Intel RDRAND、`/dev/random`），速度在 MB/s 级，物理 Monte Carlo 模拟中通常仅用于种子初始化。

### 伪随机数（Pseudo-Random Number, PRN）

> A **pseudo-random number** is a number produced by a deterministic recurrence whose output sequence is **statistically indistinguishable from a true random sequence** under standard test suites (Diehard, TestU01 BigCrush), yet is fully reproducible from a given seed. The sequence has a **finite period** after which it repeats exactly, and exhibits **vanishing autocorrelation** within its usable range — typically up to the square root of the period.

> **NIST SP 800-90A:** "A deterministic random bit generator (DRBG) is an algorithm that produces a sequence of bits from a secret initial value called a **seed**. A DRBG is often called a **pseudo-random number (or bit) generator**."

由确定性递推 $x_{n+1} = g(x_n, x_{n-1}, \ldots)$ 生成。统计特征：通过标准检验套件下**与真随机序列统计不可区分**（无显著自相关、均匀分布、高维无聚散），但具有**有限周期**（好的 PRNG 周期可到 $2^{19937}-1$），且周期内无显著长程关联。同一种子 → 同一序列，速度 GB/s 级，是计算物理中 Monte Carlo 方法的主力。

### 准随机数（Quasi-Random Number / Low-Discrepancy Sequence, LDS）

> A **quasi-random number** (or **low-discrepancy sequence**) is a deterministically constructed sequence designed to fill a multidimensional space as uniformly as possible. Unlike true and pseudo-random sequences, successive points are **negatively correlated** — each new point avoids previously occupied regions — so as to minimize discrepancy $D_N^* = O((\ln N)^d / N)$. The sequence **fails standard randomness tests** by design, since its defining virtue is deterministic uniformity rather than statistical independence.

> **Niederreiter (1992, _Random Number Generation and Quasi-Monte Carlo Methods_, SIAM):** "A low-discrepancy sequence is a deterministically constructed sequence of points in the $s$-dimensional unit cube whose star-discrepancy satisfies $D_N^* \leq C_s (\log N)^s / N$ — neither random nor pseudo-random, but intentionally more uniformly distributed."

由确定性序列构造生成（Halton、Sobol 等）。统计特征：连续点**负相关**（新点排斥已有点的位置），刻意填补空隙以最小化星差异度（Star Discrepancy $D_N^* = O((\ln N)^d/N)$）；**必然通不过随机性检验**——其价值恰在于确定性均匀而非统计独立。仅适用于高维确定性积分，不可用于随机过程模拟。

### 对比

|                   | 真随机 (True)   | 伪随机 (Pseudo)   | 准随机 (Quasi)   |
| :---------------- | :-------------- | :---------------- | :--------------- |
| **来源**          | 物理过程        | 确定性递推        | 确定性序列构造   |
| **可复现性**      | 否              | 是（同种子）      | 是               |
| **独立性 / 结构** | 独立            | 近独立            | 点间有结构       |
| **周期**          | 无              | 有限              | 无（有退化）     |
| **差异度收敛**    | $O(1/\sqrt{N})$ | $O(1/\sqrt{N})$   | $O((\ln N)^d/N)$ |
| **速度**          | MB/s            | GB/s              | GB/s             |
| **适用**          | 种子、密码学    | MC 模拟、随机过程 | 确定性高维积分   |

## 伪随机数生成算法（PRNG Algorithms）

一个好的 PRNG 需满足：长周期、高维均匀性、统计无关联、计算效率。
...
### 自由连接链模型（Freely Jointed Chain Model, FJC）

自由连接链是最基本的聚合物模型：$N$ 个刚性键，每个长度 $\ell$，键与键之间由完全自由的关节连接——每个键矢量的取向在球面上均匀分布，键与键之间无任何关联。

定义键矢量 $\mathbf{r}_i$（$i = 1, \ldots, N$），$|\mathbf{r}_i| = \ell$。由于关节完全自由：

$$\langle \mathbf{r}_i \rangle = 0, \quad \langle \mathbf{r}_i \cdot \mathbf{r}_j \rangle = \ell^2 \delta_{ij}$$

端到端矢量 $\mathbf{R} = \sum_{i=1}^N \mathbf{r}_i$。均方末端距：

$$\langle R^2 \rangle = \sum_{i=1}^N \sum_{j=1}^N \langle \mathbf{r}_i \cdot \mathbf{r}_j \rangle = \sum_{i=1}^N \langle \mathbf{r}_i^2 \rangle = N\ell^2$$

$$\boxed{\langle R^2 \rangle_{\text{FJC}} = N\ell^2}$$

### 自由旋转链模型（Freely Rotating Chain Model, FRC）

FJC 忽略了化学键的真实几何约束。自由旋转链加入了一个约束：相邻键之间的夹角固定为 $\theta$（键角），但绕键轴的二面角完全自由旋转。

此时相邻键矢量存在关联：

$$\langle \mathbf{r}_i \cdot \mathbf{r}_{i+1} \rangle = \ell^2 \cos\theta$$

对于一个键与隔 $k$ 个键的后继者，每次绕一个键旋转时，垂直于键的分量均匀分布后均值为零，只有沿键方向的分量保留——每次投影贡献因子 $\cos\theta$。重复 $k$ 次：

$$\langle \mathbf{r}_i \cdot \mathbf{r}_{i+k} \rangle = \ell^2 \cos^k\theta$$
...
### 均方末端距与均方回转半径之比 $\langle R^2 \rangle / \langle R_g^2 \rangle = 6$

**回转半径（Radius of Gyration）** 度量链中所有单体相对于质心的展宽：
...
$$\langle R_g^2 \rangle = \frac{\ell^2}{2N^2} \sum_{i=1}^N \sum_{j=1}^N |i - j|$$

计算双重求和。对固定的 $k = |i-j|$，在 $N \times N$ 的和中有 $2(N-k)$ 对 $(i,j)$（$i<j$ 和 $i>j$ 各 $(N-k)$ 对，$k \geq 1$）；$k=0$ 项贡献为零：

$$\sum_{i,j=1}^N |i - j| = 2\sum_{k=1}^{N-1} (N-k)k = 2\left(N\sum_{k=1}^{N-1} k - \sum_{k=1}^{N-1} k^2\right)$$

$$= 2\left(N \cdot \frac{N(N-1)}{2} - \frac{(N-1)N(2N-1)}{6}\right) = \frac{N(N^2 - 1)}{3}$$

代回：

$$\langle R_g^2 \rangle_{\text{FJC}} = \frac{\ell^2}{2N^2} \cdot \frac{N(N^2 - 1)}{3} = \frac{\ell^2 (N^2 - 1)}{6N}$$

在 $N \gg 1$ 的极限下：

$$\langle R_g^2 \rangle_{\text{FJC}} \xrightarrow{N \gg 1} \frac{N\ell^2}{6}$$

与 $\langle R^2 \rangle = N\ell^2$ 对比：

$$\boxed{\frac{\langle R^2 \rangle}{\langle R_g^2 \rangle} = \frac{N\ell^2}{N\ell^2/6} = 6}$$

**对 FRC 的证明**：在 FRC 中，$\langle R_{ij}^2 \rangle = \ell^2 \frac{1+\cos\theta}{1-\cos\theta}\,|i-j|$（对大的 $|i-j|$，关联按 $\cos^k\theta$ 衰减，$k = |i-j|$，前面的推导给出每步有效步长 $\tilde\ell^2 = \ell^2(1+\cos\theta)/(1-\cos\theta)$）。代入同样的双重求和：

$$\langle R_g^2 \rangle_{\text{FRC}} \xrightarrow{N \gg 1} \frac{N}{6} \cdot \ell^2 \frac{1+\cos\theta}{1-\cos\theta} = \frac{\langle R^2 \rangle_{\text{FRC}}}{6}$$

**比值为 6 具有普适性**：只要链足够长，任何满足中心极限定理的线性链（每个键矢量的关联长度有限、不存在长程记忆）都有 $\langle R^2 \rangle = Nb^2$（$b$ 为 Kuhn 长度）和 $\langle R_g^2 \rangle \approx Nb^2/6$——比值 $6$ 与局部化学细节无关，是高斯链的几何恒等式。6 这个数纯粹来自于 $R_g^2$ 定义中的双重求和组合因子 $1/3$（即 $N^3/3$ 主导）除以 $1/2$（$R_g^2$ 公式的系数）——$3 \times 2 = 6$。
