...
**优点**：
- **有解析解**——正规方程 $(\mathbf{A}^T\mathbf{A})\mathbf{a} = \mathbf{A}^T\mathbf{y}$ 一步到位，不需要迭代，不存在收敛发散的问题。
- 预测是线性的——参数 $\mathbf{a}$ 的后验协方差矩阵 $(\mathbf{A}^T\mathbf{W}\mathbf{A})^{-1}$ 精确给出每个参数的不确定度和参数间的相关性。
- $\chi^2$ 统计量提供全局拟合优度检验——$\chi^2_\nu \approx 1$ 是好拟合的数学判据，有明确的假阳/假阴率。
- **等价于高斯噪声下的最大似然估计**——在误差分布正确的条件下，线性最小二乘给出最小方差无偏估计（Gauss-Markov 定理）。

**缺点**：
- **正规矩阵 $\mathbf{A}^T\mathbf{A}$ 可能严重病态**——如 Hilbert 矩阵在幂基下 $\kappa \sim 10^{16}$（$n=12$），双精度下不可解。需要用正交基或 SVD 替代。
- 模型受限于参数的线性组合——当物理模型天然非线性（如指数衰减 $A e^{-t/\tau}$ 中的 $\tau$），不能直接使用线性最小二乘。常通过对数变换 $\ln y = \ln A - t/\tau$ 强行线性化——但这会扭曲误差分布（等方差假设被破坏），导致拟合偏差。
- **对异常值（Outliers）敏感**——残差的平方惩罚将大偏差剧烈放大（平方化），单个离群点可主导整个拟合结果。替代方案为 L1 最小化（最小绝对值）或 Huber 损失（M-估计）。
- 正规方程的数值稳定性取决于 $\mathbf{A}$ 的条件数平方——$\kappa(\mathbf{A}^T\mathbf{A}) = \kappa(\mathbf{A})^2$。当 $\mathbf{A}$ 接近秩亏时，这是致命的。SVD 解（`svdfit()`）直接处理 $\mathbf{A}$，避开了平方条件数的惩罚。

### 最大似然估计（MLE）

**优点**：
- **框架的普遍性**——不只是高斯噪声。Poisson 噪声（计数实验）、指数噪声、甚至任意自定义分布都能通过改写似然函数来处理。最小二乘只是 MLE 的一个特例。
- 统计最优性——在适当的正则条件下，MLE 是渐近有效率的（达到 Cramér-Rao 下界）、一致的（随 $m \to \infty$ 收敛到真值）。
- 参数误差的估计来自 Fisher 信息矩阵——无需额外的 Monte Carlo 或 Bootstrap。

**缺点**：
- 最大化 $\ell(\mathbf{a})$ 通常需要非线性数值优化（特别是噪声非高斯时）——没有像正规方程那样的解析解。收敛到局部极大值的风险存在。
- 模型误指定（Model Misspecification）时，MLE 的最优性丧失——若真实噪声分布与假设不同，MLE 可以有偏且不一致。
- 小样本下 MLE 的渐近性质不成立——物理中某些实验只有几十个数据点，MLE 可能有显著的有限样本偏差。
- 数值上，似然函数的乘积 $\prod p_i$ 极易下溢——需始终用对数似然 $\ell = \sum \ln p_i$，且在实现中注意 $\ln(0)$ 的防护。

### 非线性最小二乘 + Gauss-Newton

**优点**：
- 在接近极小值时收敛极快（近二次）——不需要 Hessian 矩阵（只需 Jacobian $\mathbf{J}$），比纯 Newton 法的信息需求少。
- Jacobian 结构清晰——每个元素 $\partial f(x_i)/\partial a_j$ 的物理意义明确，易于手动推导或中心差分计算。
- 线性化后的子问题仍是标准最小二乘结构——可用线性理论的误差分析和协方差传播。

**缺点**：
- **远离极小值时线性近似可能极差**——$\delta\mathbf{a}$ 过大，越过真正的极小值。$\chi^2$ 不保证下降。
- $\mathbf{J}^T\mathbf{W}\mathbf{J}$ 可近奇异——当参数空间在局部接近平坦方向时（如拟合中两个参数的效应几乎共线），Gauss-Newton 步长趋于无穷大或不精确。
- 实用中几乎从不单独使用 Gauss-Newton——退步到梯度下降的需要（即 LM 法存在的理由）是如此普遍。

### Levenberg-Marquardt（LM）法

**优点**：
- **Gauss-Newton 的速度 + 梯度下降的健壮性**——$\lambda$ 参数的无缝插值使 LM 法是求解非线性最小二乘的黄金标准。
- **$\lambda$ 自适应**——下降时减小（向 Gauss-Newton 靠拢），上升时增大（向梯度下降靠拢）。不需要手动调参。
- $\mathbf{D} = \operatorname{diag}(\mathbf{J}^T\mathbf{W}\mathbf{J})$ 的自动尺度适应——平坦方向的步长被允许更大，陡峭方向被阻尼抑制。参数空间中的各向异性得到自动处理。
- 收敛相对不敏感于初始猜测——LM 法的收敛盆地远大于纯 Gauss-Newton。
- 接近极小值时退化为 Gauss-Newton（二次收敛），保证最终的高精度。

**缺点**：
- 当 $\lambda$ 大时，LM 退化为带 $\mathbf{D}$ 度量的梯度下降——仍是线性收敛（慢）。若初始点距离极小值很远，可能需要多次加大 $\lambda$ → 小步下降 → 减小 $\lambda$ → 步长又太小的来回试探。
- 在大规模问题（$n > 10^4$ 参数，如神经网络）中解 $(\mathbf{J}^T\mathbf{W}\mathbf{J} + \lambda\mathbf{D})$ 的线性系统成本高——LM 在参数维度上不如随机梯度类方法可扩展。
- LM 是局部优化方法——它寻找最近的局部极小值，而非全局极小值。当 $\chi^2$ 曲面有多个谷（参数简并、物理模型的周期性项），LM 可能收敛到非全局的局部极小值。
- **需要 Jacobian**——对于计算昂贵的物理模型（如每次函数求值需要运行一次分子动力学模拟），即使 Jacobian 通过差分近似，代价也极为高昂。

### 使用场景决策树

1. **模型关于参数线性 + 误差为高斯** → 线性最小二乘（优先 SVD 求解，而非直接正规方程）。
2. **模型线性 + 误差非高斯** → MLE + 针对似然函数的优化（定义正确的 $\ell(\mathbf{a})$）。
3. **模型非线性 + $n \leq 100$ 参数** → Levenberg-Marquardt（标准工具）。
4. **模型非线性 + 函数求值昂贵** → 考虑响应曲面替代模型或贝叶斯优化。
5. **含离群点数据** → 用 Robust 拟合（Huber / 最小绝对值 LAD）替代普通最小二乘。
6. **双变量都有误差** → 正交距离回归（ODR）。
