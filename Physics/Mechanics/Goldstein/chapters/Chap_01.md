...
¶一位矢为$\bm{r}_1,\cdots,\bm{r}_{n}$的质点系，其完整约束(holomonic constraint)写为
...
¶一个质点系，在约束力不做功的前提下，要求对于任意的虚位移$\delta\bm{r}^{(i)}$都有
...
¶在选择了相互独立、与自由度相适应的广义坐标$\bm{r}^{(i)}=\bm{r}^{(i)}(q^{1},\cdots,q^{n},t)$后，定义广义力分量
...
¶此式可以被改写为*Nielsen*形式，因为对于$T(q^{j},\dot{q}^{j},t)$
...
¶当$U=U(q^j,\dot{q}^{j})$时，(3)式仍适用，如电磁场的势

$$
    U=q\phi-q\bm{A}\cdot\bm{v}.
$$

¶在*Stokes*型阻力$\bm{F}_{f}^{(i)}=-(k_{x}^{(i)}\bm{v}_{x}+k_{y}^{(i)}\bm{v}_{y}+k_{z}^{(i)}\bm{v}_{z})$的情形下，定义系统的*Rayleigh*耗散函数
...
$$
Q^{(f)}_{j}=\sum_{i}\bm{F}_{f}^{(i)}\cdot\frac{\partial \bm{r}}{\partial q^j}=\sum_{i}-\nabla_{v}\mathcal{F}^{(i)}\cdot\frac{\partial \dot{\bm{r}}}{\partial \dot{q}^j}=-\sum_{i}\frac{\partial \mathcal{F}^{(i)}}{\partial \dot{q}^j}=-\frac{\partial \mathcal{F}}{\partial \dot{q}^j},
$$
...
$$
\color{Teal}
Q_{j}=-\frac{\delta L}{\delta q^j}+\frac{\partial \mathcal{F}}{\partial \dot{q}^j}.
$$
