...
\par定义质心位矢$\bm{R}=(m_{1}\bm{r}_{1}+m_{2}\bm{r}_{2})/(m_{1}+m_{2})$和差位矢$\bm{r}=\bm{r}_{2}-\bm{r}_{1}$，则
$$
\begin{aligned}
...
\end{aligned}
$$
\par假设两质点间为向心力，即服从强作用与反作用定理.选定一点为原点，考察另一点的运动
$$
L=\frac{1}{2}m(\dot{r}^2+r^2\dot{\theta}^2)-V(r),
$$
立刻得到角动量守恒
$$
p_{\theta}=\part{L}{\dot{\theta}}=mr^2\dot{\theta}=l,
$$
也有能量守恒
$$
\begin{equation}
\dot{r}\part{L}{\dot{r}}+\dot{\theta}\part{L}{\dot{\theta}}-L=\frac{1}{2} m\dot{r}^2+\frac{l^2}{2mr^2}+V(r)=E.
\end{equation}
$$
\par求解轨迹$r(\theta)$时作代换
$$
\deri{}{t}=\dot{\theta}\deri{}{\theta}=\frac{l}{mr^2}\deri{}{\theta},
$$
得到
$$\begin{equation}\color{Teal}
\upd\theta=\frac{l\upd r}{mr^2\sqrt{\frac{2}{m}(E-V(r)-\frac{l^2}{2mr^2})}}.\end{equation}
$$
...
\par考虑
$$
\deri{}{t}(\bm{r}^{(i)}\cdot\bm{p}^{(i)})=2T+\bm{r}^{(i)}\cdot\bm{F}^{(i)},
$$
取其在$[0,\tau]$时间的平均
$$
...
$$
应用见[推导理想气体状态方程](../examples/Chap_03.md#推导理想气体状态方程).
\par特别地，对于单质点系统，且受向心力$-\partial V/\partial r=ar^{n}$
$$
\begin{equation}
...
a>0,\ n>-3.
$$
\par在$r=r_{0}$附近将(1)式展开至二阶
$$
\begin{aligned}
&\quad\frac{1}{2}m\left[\deri{}{t}(r-r_{0})\right]^2-\frac{l^2}{mr_{0}^3}(r-r_{0})+\frac{3l^2}{2mr_{0}^{4}}(r-r_{0})^{2}+V'(r_{0})(r-r_{0})+\frac{1}{2}V''(r_{0})(r-r_{0})^{2}\\
&=\frac{1}{2}m\left[\deri{}{t}(r-r_{0})\right]^2+\frac{3l^2}{2mr_{0}^{4}}(r-r_{0})^{2}+\frac{1}{2}V''(r_{0})(r-r_{0})^{2}\\
&=E-V_{\text{eff}}(r_{0}),
\end{aligned}
$$
...
则角频率$\omega_{\theta}$满足
$$
\omega_{\theta}^2=\frac{\omega_{t}^{2}}{\dot{\theta}^2}=3+\frac{mr_{0}^{4}}{l^2}V''(r_{0})=3+\left.\frac{\upd\ln V'(r)}{\upd\ln r}\right|_{r=r_{0}},
$$
得到
$$
...
\par对于符合平方反比定律的力$V(r)=-k/r$，求解(2)式
$$
\begin{aligned}
\theta-\theta_{0}&=\int_{r_{0}}^{r}\frac{l\upd r}{mr^2\sqrt{\frac{2}{m}(E+\frac{k}{r}-\frac{l^2}{2mr^2})}}\\
&\overset{u=1/r}{=}\int_{u_{0}}^{u}\frac{\upd u}{\sqrt{\frac{2mE}{l^2}+\frac{2mk}{l^2}u-u^2}}\\
&\equiv\int_{u_{0}}^{u}\frac{\upd u}{\sqrt{\alpha+\beta u-u^2}}\\
...
\par由角动量守恒和(2)式得到
$$
t-t_{0}=\int_{r_{0}}^{r}\frac{\upd r}{\sqrt{\frac{2}{m}(E+\frac{k}{r}-\frac{l^2}{2mr^2})}},
$$
引入近偏点角作为计算辅助量
$$
r=a(1-e\cos\psi),
...
并将$E$与$l$用$a$与$e$表示
$$
t-t_{0}=\sqrt{\frac{ma^3}{k}}\int_{\psi_{0}}^{\psi}1-e\cos\psi\ \upd\psi,
$$
积分一周得到
$$
...
\par参见[卢瑟福散射](../append/Chap_03/卢瑟福散射.pdf).
