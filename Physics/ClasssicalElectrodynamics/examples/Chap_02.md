...
$$
    \begin{aligned}
    \varphi_{K}&=\frac{1}{4\pi\varepsilon_0}\int_{S}\frac{-\sigma_{e}\mathrm{d} f'}{|\bm{r}_{K}-\bm{r}'|}+\frac{\sigma_{e}\mathrm{d} f'}{|\bm{r}_{K}-(\bm{r}'+\bm{d})|}\\
    &=\frac{1}{4\pi\varepsilon_0}\int_{S}\mathrm{d} f'\sigma_{e}(-d\bm{e}_{n})\cdot\nabla\frac{1}{|\bm{r}_{K}-\bm{r}'|}\\
    &=-\frac{D_{e}}{4\pi\varepsilon_0}\int_{S}\mathrm{d}\bm{f}'\cdot\nabla\frac{1}{|\bm{r}_{K}-\bm{r}'|}\\
    &=-\frac{D_{e}}{\varepsilon_0}\frac{\Omega_{K}}{4\pi},
    \end{aligned}
$$

其中称$D_{e}=\sigma_{e}d$为电偶极子面密度，$\Omega_{K}$为$S$相对于$K$的立体角，其微元$\mathrm{d}\Omega_{K}$与$\mathrm{d}\bm{f}'\cdot(\bm{r}'-\bm{r}_{K})$符号相同.对于无穷延伸的电偶极子层或闭合电偶极子层
...
¶设导体组有$n$个导体$\Sigma_{i},\ i=1,\cdots,n$，其电势为$U_{1},\cdots,U_{n}$，导体间介质的电荷分布$\rho_{f}$，设电势可分解为
...
$$
Q_{i}=-\int_{\partial\Sigma_{i}}\frac{\partial \varphi}{\partial n}\ \mathrm{d} f'=-\int_{\partial\Sigma_{i}}\frac{\partial \varphi_{0}}{\partial n}\ \mathrm{d} f'+\sum_{j=1}^{n}\left(-\int_{\partial\Sigma_{i}}\frac{\partial \chi_{j}}{\partial n}\ \mathrm{d} f'\right)U_{j}\equiv q_{i}+\sum_{i=1}^{n}C_{ij}U_{j},
$$
...
¶点电四极子由一组相距$\bm{d}\ (d\to0)$的反向点电偶极子构成，其电荷分布
...
$$
\begin{aligned}
\mathcal{Q}_{ij}&=\int_{V}\rho_{q}(\bm{r}')(3x'_ix'_j-\delta_{ij}(r')^2)\ \mathrm{d}^3\bm{r}'\\
...
$$
\begin{aligned}
W&=\int_{0}^{\pi/2}(\bm{p}\times\bm{E})\cdot\ \hat{\bm{e}}_{x}\mathrm{d}\theta
\\&=\frac{1}{4\pi\varepsilon_{0}}\frac{p^2}{(2a)^3}\int_{0}^{\pi/2}\sin\theta\cos\theta\ \mathrm{d}\theta\\
...
&\left.\left(\varepsilon_{2}\frac{\partial \varphi_{2}}{\partial z}-\varepsilon_{1}\frac{\partial \varphi_{1}}{\partial z}\right)\right|_{z=0}=0,
\end{aligned}
$$
...
&=(\varepsilon_{2}-\varepsilon_{0})\frac{\partial \varphi_{2}}{\partial z}-(\varepsilon_{1}-\varepsilon_{0})\frac{\partial \varphi_{1}}{\partial z}\\
&=\varepsilon_{0}\left(\frac{\partial \varphi_{1}}{\partial z}-\frac{\partial \varphi_{2}}{\partial z}\right)\\
...
$$
\begin{aligned}
Q_{p}&=\int_{S}\sigma_{p}\ \mathrm{d} f=2\pi\int_{0}^{\infty}\frac{\varepsilon_{0}}{4\pi\varepsilon_{2}}\frac{\varepsilon_{2}-\varepsilon_{1}}{\varepsilon_{1}+\varepsilon_{2}}\frac{2aQ}{(a^2+\rho^2)^{3/2}}\ \rho\mathrm{d}\rho\\
...
$$
\varphi(r,\theta)=\left\{\begin{aligned}&\sum_{n=0}^{\infty}\left(a_{n}r^{n}+\frac{b_{n}}{r^{n+1}}\right)P_{n}(\cos\theta),\ r\leq r_0,\\
...
&\left.\varepsilon_{0}\frac{\partial \varphi}{\partial r}\right|_{r_0^+}-\varepsilon\left.\frac{\partial \varphi}{\partial r}\right|_{r_0}=0,
...
$$
\varphi(r,\theta)=\left\{\begin{aligned}
