...
<mark type="thm">*Lagrange*定理</mark> 构造两个左陪集$r_{j}\cdot\mathcal{H}$和$r_{k}\cdot\mathcal{H}\ (r_{j}\neq r_{k}),$ 则要么两个陪集相同, 要么没有公共元素, 这是因为
...
==自同构群== 记$\operatorname{Aut}(\mathcal{G})=\{\varphi:\mathcal{G}\to\mathcal{G}\mid\varphi\ \text{is\ isomorphism}\}$, 在复合映射下, $\operatorname{Aut}(\mathcal{G})$构成群, 称为自同构群.
...
==半直积== 在映射$\varphi:\mathcal{K}\to\operatorname{Aut}(\mathcal{G}),\,\varphi(k):g\mapsto g'$下, 在$\mathcal{G}\times\mathcal{K}$上定义乘法
